...
#ifndef _MAX_MESHES_AMOUNT_
#define _MAX_MESHES_AMOUNT_ (128)
#endif

#ifndef _MAX_ANIMATIONS_AMOUNT_
#define _MAX_ANIMATIONS_AMOUNT_ (128)
#endif

#define INVALID_GL_VAO (UINT32_MAX)
#define INVALID_GL_VBO (UINT32_MAX)

#define IS_MESH_INDEX_VALID(mesh_index) (mesh_index < MAX_MESHES_AMOUNT)
...
static err_t reset_mesh(mesh_t* mesh) {
    err_t err = NO_ERROR;

    CHECK(mesh != NULL);
    UNCONSTIFY(uint32_t, mesh->mesh_index) = INVALID_MESH_INDEX;
    mesh->is_static = 0;
    mesh->is_binded = 0;
    mesh->is_data_preserved = 0;
...
static err_t gl_delete_vertex_array(uint32_t gl_vao) {
    err_t err = NO_ERROR;

    CHECK(gl_vao != INVALID_GL_VAO);
...
static err_t bind_mesh_internal(uint32_t* out_gl_vao,
                                uint32_t* out_gl_vbos, vbo_data_t* vbos_data, uint32_t vbos_amount) {
    err_t err = NO_ERROR;
    
    CHECK(out_gl_vao != NULL);
    *out_gl_vao = INVALID_GL_VAO;

    CHECK(out_gl_vbos != NULL);
    CHECK(vbos_data != NULL);
    CHECK(vbos_amount > 0);

    for (uint32_t i = 0; i < vbos_amount; i++) {
        out_gl_vbos[i] = INVALID_GL_VBO;
    }

    CHECK_NO_GL_ERROR();

    glGenVertexArrays(1, out_gl_vao);
    DEBUG_CHECK_NO_GL_ERROR();
    
    glBindVertexArray(*out_gl_vao);
    DEBUG_CHECK_NO_GL_ERROR();

    glGenBuffers(vbos_amount, out_gl_vbos);
    DEBUG_CHECK_NO_GL_ERROR();
    for (uint32_t i = 0; i < vbos_amount; i++) {
        glBindBuffer(GL_ARRAY_BUFFER, out_gl_vbos[i]);
        DEBUG_CHECK_NO_GL_ERROR();

        glBufferData(GL_ARRAY_BUFFER, vbos_data[i].data_array_size, vbos_data[i].data_array, GL_STATIC_DRAW);
        DEBUG_CHECK_NO_GL_ERROR();
        
        switch (vbos_data[i].type) {
        case GL_BYTE:
        ...
        case GL_UNSIGNED_INT:
            glVertexAttribIPointer(i, vbos_data[i].size, vbos_data[i].type, vbos_data[i].stride, (void*)0);
            break;
        case GL_HALF_FLOAT:
        case GL_FLOAT:
        case GL_DOUBLE:
        ...
        case GL_UNSIGNED_INT_10F_11F_11F_REV:
            glVertexAttribPointer(i, vbos_data[i].size, vbos_data[i].type, GL_FALSE, vbos_data[i].stride, (void*)0);
            break;
        default:
            THROW();
        }
...
cleanup:
    if (IS_ERROR(err)) {
        if (out_gl_vao != NULL) {
            gl_delete_vertex_array(*out_gl_vao);
            *out_gl_vao = INVALID_GL_VAO;
        }
        if (out_gl_vbos != NULL) {
            for (uint32_t i = 0; i < vbos_amount; i++) {
...
err_t check_mesh(mesh_t* mesh) {
    ...
    if (mesh->is_static || mesh->is_binded) {
        CHECK(mesh->vbos_amount > 0);
        CHECK(mesh->gl_vbos != NULL);
    }

    if (mesh->is_static) {
        CHECK(mesh->mesh_index == INVALID_MESH_INDEX);
        CHECK(mesh->is_data_preserved);
    } else {
        CHECK(IS_MESH_INDEX_VALID(mesh->mesh_index));
        CHECK(meshes_list[mesh->mesh_index] == mesh);
        CHECK(mesh->is_data_preserved || mesh->is_binded);
    }

    if (mesh->is_data_preserved) {
        CHECK(mesh->vbos_data != NULL);
    }

    CHECK(mesh->indices_count > 0);
    CHECK(mesh->indices_array != NULL);

    if (mesh->joints_amount > 0) {
        CHECK(mesh->joints != NULL);
    }
...
err_t check_animation(animation_t* animation) {
    ...
    CHECK(IS_ANIMATION_INDEX_VALID(animation->animation_index));
    CHECK(animations_list[animation->animation_index] == animation);
...
err_t generate_mesh(mesh_t** out_mesh,
    ...
    *out_mesh = NULL;
    ...
    CHECK(vbos_data != NULL);
    CHECK(vbos_amount > 0);
    CHECK(indices_array != NULL);
    CHECK(indices_count > 0);

    CHECK(meshes_amount < MAX_MESHES_AMOUNT);

    for (uint32_t i = 0; i < MAX_MESHES_AMOUNT; i++) {
        if (meshes_list[i] == NULL) {
            mesh_index = i;
            break;
        }
    }
    CHECK(IS_MESH_INDEX_VALID(mesh_index));

    mesh = malloc(sizeof(mesh_t));
    CHECK(mesh != NULL);

    RETHROW_IF_ERROR(reset_mesh(mesh));

    mesh->vbos_amount = vbos_amount;
    mesh->gl_vbos = malloc(sizeof(uint32_t) * vbos_amount);
    CHECK(mesh->gl_vbos != NULL);
    
    if (!should_skip_binding) {
        RETHROW_IF_ERROR(bind_mesh_internal(&mesh->gl_vao, mesh->gl_vbos, vbos_data, vbos_amount));
        mesh->is_binded = 1;
    }
    
    if (should_preserve_data) {
        mesh->vbos_data = malloc(sizeof(vbo_data_t) * mesh->vbos_amount);
        CHECK(mesh->vbos_data != NULL);
        for (uint32_t i = 0; i < vbos_amount; i++) {
            mesh->vbos_data[i].data_array_size = vbos_data[i].data_array_size;
            mesh->vbos_data[i].data_array = NULL;
            mesh->vbos_data[i].size = vbos_data[i].size;
            mesh->vbos_data[i].type = vbos_data[i].type;
            mesh->vbos_data[i].stride = vbos_data[i].stride;
            mesh->vbos_data[i].divisor = vbos_data[i].divisor;
        }

        for (uint32_t i = 0; i < vbos_amount; i++) {
            mesh->vbos_data[i].data_array = malloc(mesh->vbos_data[i].data_array_size);
            CHECK(mesh->vbos_data[i].data_array != NULL);
            memcpy(mesh->vbos_data[i].data_array, vbos_data[i].data_array, mesh->vbos_data[i].data_array_size);
        }

        mesh->is_data_preserved = 1;
    }

    mesh->indices_count = indices_count;
    mesh->indices_array = malloc(sizeof(uint32_t) * indices_count);
    CHECK(mesh->indices_array != NULL);
    memcpy(mesh->indices_array, indices_array, sizeof(uint32_t) * indices_count);

    UNCONSTIFY(uint32_t, mesh->mesh_index) = mesh_index;
    meshes_list[mesh_index] = mesh;
    meshes_amount += 1;
...
err_t bind_mesh(mesh_t* mesh) {
    ...
    CHECK(mesh->is_static);
    CHECK(mesh->is_binded == 0);

    RETHROW_IF_ERROR(bind_mesh_internal(&mesh->gl_vao, mesh->gl_vbos, mesh->vbos_data, mesh->vbos_amount));

    for (uint32_t i = 0; i < mesh->vbos_amount; i++) {
        free(mesh->vbos_data[i].data_array);
        mesh->vbos_data[i].data_array = NULL;
    }
    free(mesh->vbos_data);
    mesh->vbos_data = NULL;
...
err_t generate_2d_quad_mesh(mesh_t** out_mesh,
    ...
    return generate_mesh(out_mesh, vbo_datas_arr, 2, indices_array, 6, unbinded, unbinded);
}


// TODO: revisit this hot mess
static err_t divide_ball_mesh(uint32_t polygons_amount, float** vertices_position_arr_ptr, uint32_t** indices_array_ptr) {
    err_t err = NO_ERROR;
    float* new_vertices_position_arr = NULL;
    uint32_t* new_indices_array = NULL;
    float* vertices_position_arr = NULL;
    uint32_t* indices_array = NULL;
    
    CHECK(vertices_position_arr_ptr != NULL);
    CHECK(*vertices_position_arr_ptr != NULL);
    CHECK(indices_array_ptr != NULL);
    CHECK(*indices_array_ptr != NULL);

    vertices_position_arr = *vertices_position_arr_ptr;
    indices_array = *indices_array_ptr;

    new_vertices_position_arr = malloc(sizeof(float) * polygons_amount * 4 * 3 * 3);
    CHECK(new_vertices_position_arr != NULL);
    
    new_indices_array = malloc(sizeof(uint32_t) * polygons_amount * 4 * 3);
    CHECK(new_indices_array != NULL);
    ...
    free(*vertices_position_arr_ptr);
    *vertices_position_arr_ptr = new_vertices_position_arr;
    new_vertices_position_arr = NULL;
    
    free(*indices_array_ptr);
    *indices_array_ptr = new_indices_array;
    new_indices_array = NULL;

cleanup:
    free(new_vertices_position_arr);
    new_vertices_position_arr = NULL;

    free(new_indices_array);
    new_indices_array = NULL;
...
err_t mesh_generate_ball(mesh_t** out_mesh, uint32_t divisions, uint8_t unbinded) {
    ...
    CHECK(vertices_position_arr != NULL);
    ...
    for (uint32_t divs = 0; divs < divisions; divs++) {
        RETHROW_IF_ERROR(divide_ball_mesh(polygons_amount, &vertices_position_arr, &indices_array));
    ...
    */
    ...
    vbo_data_t vbo_datas_arr[1] = {
        {
            .data_array_size = sizeof(float)*vertices_amount*3,
            .data_array = (void*)vertices_position_arr,
            .size = 3,
            .type = GL_FLOAT,
            .stride = 3*sizeof(float),
    ...
    RETHROW_IF_ERROR(generate_mesh(out_mesh, vbo_datas_arr, 1, indices_array, polygons_amount*3, unbinded, unbinded));
...
err_t mesh_from_wavefront_obj_ext(mesh_t** out_mesh, const char* obj_file_path, quat_vec_vec_t transform_qvv, uint8_t unbinded) {
    ...
    int64_t normal_index = 0;
    int64_t uv_index = 0;
    int64_t unique_vertex_index = 0;
    int64_t polygon_index = 0;
    int64_t face_first_unique_vertex_index = 0;
    ...
    uint32_t between_spaces_slash_count = 0;
    ...
    uint32_t* indices_array = NULL;
    float* vertices_position_arr = NULL;
    float* vertices_texcoord_arr = NULL;
    float* vertices_normal_arr = NULL;
    ...
    *out_mesh = NULL;
    ...
    CHECK(obj_file_path != NULL);

    file_length = load_file_contents(&obj_str,obj_file_path);
    CHECK(obj_str != NULL);
    ...
    CHECK(vertices_position_arr != NULL);
    CHECK(indices_array != NULL);
    CHECK(vertices_texcoord_arr != NULL);
    CHECK(vertices_normal_arr != NULL);
    ...
    vbo_data_t vbo_datas_arr[3] = {
        {
            .data_array_size = sizeof(float)*unique_vertices_count*3,
            .data_array = (void*)vertices_position_arr,
            .size = 3,
            .type = GL_FLOAT,
            .stride = 3*sizeof(float),
        ...
        {
            .data_array_size = sizeof(float)*unique_vertices_count*2,
            .data_array = (void*)vertices_texcoord_arr,
            .size = 2,
            .type = GL_FLOAT,
            .stride = 2*sizeof(float),
    ...
    RETHROW_IF_ERROR(generate_mesh(out_mesh, vbo_datas_arr, 3, indices_array, polygons_count*3, unbinded, unbinded));
...
cleanup:
    ...
    free(vertices_texcoord_arr);
    vertices_texcoord_arr = NULL;
    
    free(vertices_normal_arr);
...
err_t mesh_from_wavefront_obj(mesh_t** out_mesh, const char* obj_file_path, uint8_t unbinded) {
    ...
    return mesh_from_wavefront_obj_ext(out_mesh, obj_file_path, transform_qvv, unbinded);
}

// TOOD: revisit this hot mess
static err_t mesh_from_collada_dae_joint_hierarchy(char* dae_str, mesh_t* mesh) {
    err_t err = NO_ERROR;
    uint32_t i;
    int64_t str_i;
    int64_t dstr_i1;
...
err_t mesh_from_collada_dae_ext(mesh_t** out_mesh, const char* dae_file_path, quat_vec_vec_t transform_qvv, uint8_t unbinded) {
    ...
    *out_mesh = NULL;
    ...
    CHECK(dae_file_path != NULL);


    load_file_contents(&dae_str, dae_file_path);
    CHECK(dae_str != NULL);
    ...
    CHECK(vertices_positions_array != NULL);
    ...
    CHECK(vertices_normals_array != NULL);
    ...
    CHECK(vertices_texcoords_array != NULL);
    ...
    for (uint64_t i = 0; i < joints_data_count_array_length; i++) {
    ...
    vertices_joint_ids_array = malloc(sizeof(int32_t) * vertices_joint_ids_array_length);
    vertices_joint_wheights_array_length = joints_data_count_array_length * 3;
    vertices_joint_wheights_array = malloc(sizeof(float) * vertices_joint_wheights_array_length);
    CHECK(vertices_joint_ids_array != NULL);
    CHECK(vertices_joint_wheights_array != NULL);
    ...
    for (uint64_t i = 0; i < joints_data_count_array_length; i++) {
        ...
        if (joint_data_count < 3) {
            ...
            for (int32_t j = 0; j < joint_data_count; j++) {
    ...
    CHECK(indices_array != NULL);
    CHECK(vbo_vertices_position_arr != NULL);
    CHECK(vbo_vertices_texcoord_arr != NULL);
    CHECK(vbo_vertices_normal_arr != NULL);
    CHECK(vbo_vertices_joint_id_arr != NULL);
    CHECK(vbo_vertices_joint_wheight_arr != NULL);
    ...
    vbo_data_t vbo_datas_arr[5] = {
        {
            .data_array_size = sizeof(float)*triangles_count*3*3,
            .data_array = (void*)vbo_vertices_position_arr,
            .size = 3,
            .type = GL_FLOAT,
            .stride = 3*sizeof(float),
            .divisor = 0
        ...
        {
            .data_array_size = sizeof(float)*triangles_count*3*2,
            .data_array = (void*)vbo_vertices_texcoord_arr,
            .size = 2,
            .type = GL_FLOAT,
            .stride = 2*sizeof(float),
            .divisor = 0
        ...
        {
            .data_array_size = sizeof(float)*triangles_count*3*3,
            .data_array = (void*)vbo_vertices_normal_arr,
            .size = 3,
            .type = GL_FLOAT,
            .stride = 3*sizeof(float),
            .divisor = 0
        ...
        {
            .data_array_size = sizeof(int32_t)*triangles_count*3*3,
            .data_array = (void*)vbo_vertices_joint_id_arr,
            .size = 3,
            .type = GL_INT,
        ...
        {
            .data_array_size = sizeof(float)*triangles_count*3*3,
            .data_array = (void*)vbo_vertices_joint_wheight_arr,
            .size = 3,
            .type = GL_FLOAT,
            .stride = 3*sizeof(float),
            .divisor = 0
    ...
    RETHROW_IF_ERROR(generate_mesh(&mesh, vbo_datas_arr, 5, indices_array, triangles_count*3, unbinded, unbinded));
...
err_t mesh_from_collada_dae(mesh_t** out_mesh, const char* dae_file_path, uint8_t unbinded) {
    ...
    return mesh_from_collada_dae_ext(out_mesh, dae_file_path, transform_qvv, unbinded);
}

static err_t reset_animation(animation_t* animation) {
    err_t err = NO_ERROR;

    CHECK(animation != NULL);

    UNCONSTIFY(uint32_t, animation->animation_index) = INVALID_ANIMATION_INDEX;
    animation->is_static = 0;
    animation->joints_amount = 0;
    animation->joints_key_frames = NULL;
    ...
    if (anim != NULL) {
        if (!anim->is_static) {
            for (uint32_t i = 0; i < anim->joints_amount; i++) {
                ...
                anim->joints_key_frames[i].key_frames = NULL;
            }
        
            free(anim->joints_key_frames);
            anim->joints_key_frames = NULL;
        }
...
err_t animation_from_collada_dae_ext(animation_t** out_animation, const char* dae_file_path, joint_t* joints, uint32_t joints_amount, quat_vec_vec_t transform_qvv) {
    ...
    uint32_t animation_index = INVALID_ANIMATION_INDEX;
    char* dae_str = NULL;
    int64_t str_i = 0;
    int64_t dstr_i = 0;
    char* name = NULL;
    int32_t joint_i = 0;
    float* key_frames_time_stamp = NULL;
    uint32_t key_frames_transform_matrices_array_length = 0;
    float* key_frames_transform_matrices_array = NULL;
    mat4_t transform_mat = {0};

    CHECK(out_animation != NULL);
    *out_animation = NULL;

    CHECK(dae_file_path != NULL);
    CHECK(joints != NULL);
    CHECK(joints_amount > 0);

    CHECK(animations_amount < MAX_ANIMATIONS_AMOUNT);

    for (uint32_t i = 0; i < MAX_ANIMATIONS_AMOUNT; i++) {
        if (animations_list[i] == NULL) {
            animation_index = i;
            break;
        }
    }
    CHECK(IS_ANIMATION_INDEX_VALID(animation_index));
    
    transform_mat = mat4_from_quat_vec_vec(transform_qvv);
    load_file_contents(&dae_str, dae_file_path);
    CHECK(dae_str != NULL);
    ...
    RETHROW_IF_ERROR(reset_animation(anim));

    anim->joints_amount = joints_amount;
    anim->joints_key_frames = calloc(1, sizeof(joint_key_frame_t) * joints_amount);
    CHECK(anim->joints_key_frames != NULL);

    for (uint32_t i = 0; i < joints_amount; i++) {
        anim->joints_key_frames[i].key_frames = NULL;
        anim->joints_key_frames[i].key_frames_amount = 0;
    }
    ...
    while (1) {
        ...
        CHECK(key_frames_time_stamp != NULL);

        for (uint32_t i = 0; i < anim->joints_key_frames[joint_i].key_frames_amount; i++) {
            anim->joints_key_frames[joint_i].key_frames[i].time_stamp = key_frames_time_stamp[i];
        }

        free(key_frames_time_stamp);
        key_frames_time_stamp = NULL;
        ...
        key_frames_transform_matrices_array = NULL;
    }

    // find local matrix for joints without animation
    for (uint32_t i = 0; i < joints_amount; i++) {
        if (anim->joints_key_frames[i].key_frames_amount != 0) {
            continue;
        }

        str_i = str_find_substr(dae_str, "<visual_scene");
        str_i += str_find_substr(&(dae_str[str_i]), joints[i].name);
        ...
        memcpy(
            anim->joints_key_frames[i].key_frames[0].joint_local_transform.mat,
            key_frames_transform_matrices_array, sizeof(float) * 16
        );
        
        // apply transform
        if (i == 0) {
            anim->joints_key_frames[i].key_frames[0].joint_local_transform = mat4_mul(
                transform_mat,
                anim->joints_key_frames[i].key_frames[0].joint_local_transform
            );
        }
        
        anim->joints_key_frames[i].key_frames[0].joint_local_transform_qvv = quat_vec_vec_from_mat4(anim->joints_key_frames[i].key_frames[0].joint_local_transform);

        free(key_frames_transform_matrices_array);
        key_frames_transform_matrices_array = NULL;
    }

    UNCONSTIFY(uint32_t, anim->animation_index) = animation_index;
    animations_list[animation_index] = anim;
    animations_amount += 1;
...
err_t animation_from_collada_dae(animation_t** out_animation, const char* dae_file_path, joint_t* joints, uint32_t joints_amount) {
    ...
    return animation_from_collada_dae_ext(out_animation, dae_file_path, joints, joints_amount, transform_qvv);
}

static quat_vec_vec_t mix_joint_local_transform_qvvs(quat_vec_vec_t qvv1, quat_vec_vec_t qvv2, float mix) {
...
static quat_vec_vec_t get_joint_local_transform_qvv_at_animation_time(animation_t* anim, uint32_t joint_i, float time_stamp) {
...
err_t pose_mesh_mix_from_animation(mesh_t* mesh, animation_t* anim, float time_stamp, float mix) {
    ...
    CHECK(mesh->joints_amount == anim->joints_amount);

    mix = fmax(fmin(mix, 1), 0); // clamp between 0 and 1

    for (uint32_t joint_i = 0; joint_i < anim->joints_amount; joint_i++) {
...
err_t pose_mesh_set_from_animation(mesh_t* mesh, animation_t* anim, float time_stamp) {
    return pose_mesh_mix_from_animation(mesh, anim, time_stamp, 1);
}

static err_t draw_mesh_internal(mesh_t* mesh, uint32_t instance_count) {
    err_t err = NO_ERROR;

    // no need to CHECK_MESH, will only be called after it
    CHECK(mesh != NULL);
...
err_t draw_mesh_instanced(mesh_t* mesh, uint32_t instance_count) {
    ...
    if (mesh->joints_amount > 0) {
        // TODO: mallocing for each draw is slow. we can malloc at creation and reuse the buffer.
        float* joints_transform_matrices = malloc(sizeof(float) * 16 * mesh->joints_amount);
        CHECK(joints_transform_matrices != NULL);
        ...
        DEBUG_CHECK_NO_GL_ERROR();
        glUniformMatrix4fv(current_shader->u_joint_matrices_loc, mesh->joints_amount, GL_FALSE, joints_transform_matrices);
        DEBUG_CHECK_NO_GL_ERROR();
        
        free(joints_transform_matrices);
        joints_transform_matrices = NULL;
    }

    RETHROW_IF_ERROR(draw_mesh_internal(mesh, instance_count));
...
err_t draw_mesh(mesh_t* mesh) {
    return draw_mesh_instanced(mesh, 1);
}
...
err_t draw_mesh_posed_instanced(mesh_t* mesh, uint32_t instance_count) {
    ...
    uint32_t joint_i = 0;
    joint_t* joint = NULL;
    mat4_t final_joint_transform_mat = {0};
    ...
    // set pose
    DEBUG_CHECK_NO_GL_ERROR();
    glUniformMatrix4fv(current_shader->u_joint_matrices_loc, mesh->joints_amount, GL_FALSE, mesh->pose_joint_transform_matrices);
    DEBUG_CHECK_NO_GL_ERROR();
...
err_t draw_mesh_posed(mesh_t* mesh) {
    return draw_mesh_posed_instanced(mesh, 1);
}
...
err_t destroy_mesh(mesh_t* mesh) {
    ...
    CHECK_MESH(mesh);
    meshes_list[mesh->mesh_index] = NULL;
    meshes_amount -= 1;
...
err_t destroy_animation(animation_t* anim) {
    ...
    animations_list[anim->animation_index] = NULL;
    animations_amount -= 1;
...
void save_mesh_to_c_file(mesh_t* mesh, const char* name, const char* file_path) {
    ...
    for (uint64_t i = 0; i < mesh->vbos_amount; i++) {
        ...
        for (uint64_t j = 0; j < mesh->vbos_data[i].data_array_size; j++) {
            if (j != 0) fprintf(fp,", ");
            if (j%20 == 0) fprintf(fp,"\n\t");
            fprintf(fp,
                "%hhu",
                ((uint8_t*)(mesh->vbos_data[i].data_array))[j]
            );
        }
        fprintf(fp, "\n};\n");
    }
    ...
    // mesh_vbos_data
    fprintf(fp,
        ""      "static vbo_data_t %s_mesh_vbos_data[] = {\n"
        ,
        name
    );
    for (uint64_t i = 0; i < mesh->vbos_amount; i++) {
        ...
        fprintf(fp,
            "\t"        "{\n"
            "\t\t"          ".data_array_size = %u,\n"
            "\t\t"          ".data_array = %s_mesh_vbo_data_array%llu,\n"
            "\t\t"          ".size = %d,\n"
            "\t\t"          ".type = %d,\n"
            "\t\t"          ".stride = %d,\n"
            "\t\t"          ".divisor = %u\n"
            "\t"        "}"
            ,
            mesh->vbos_data[i].data_array_size,
            name,
            i,
            mesh->vbos_data[i].size,
            mesh->vbos_data[i].type,
            mesh->vbos_data[i].stride,
            mesh->vbos_data[i].divisor
        );
    }
    fprintf(fp,
        ""      "\n};\n"
