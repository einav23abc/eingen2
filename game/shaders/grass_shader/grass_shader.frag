...
float smooth_round(float x, float smooth_part_size) {
    float fract_x = fract(x);
    float half_smooth_part_size = smooth_part_size * 0.5;
    float is_smooth_part = floor(fract_x + 0.5 + half_smooth_part_size) * floor(1.5 + half_smooth_part_size - fract_x);
    float steps = floor(x + 0.5 - half_smooth_part_size);
    float smooth_part = sin(x * M_PI / smooth_part_size) * 0.5 + 0.5;
    return smooth_part * is_smooth_part + steps;
}

float MAX_BLADE_HEIGHT = 4.0;
float MAX_PATCH_HEIGHT = 8.0;
float MAX_HEIGHT = MAX_BLADE_HEIGHT + MAX_PATCH_HEIGHT;
float height_map(vec2 p) {
    float patch_height = smooth_round(layered_noise(p), 0.125) * MAX_PATCH_HEIGHT;
    ...
    float blade_height = (cos(p.x) * 0.5 + 0.5) * (cos(p.y) * 0.5 + 0.5) * MAX_BLADE_HEIGHT;

    return patch_height + blade_height;
}
...
void main(){
    ...
    // a fix? the other one was not good
    parallax_step = -(view_dir / view_dir.y) * MAX_HEIGHT / float(PARALAX_ITERATIONS);
    ...
    float mag_b = sqrt(dot(b, b));
    float mag_a = sqrt(dot(a, a));
    float ratio = mag_b / (mag_b + mag_a);
    vec3 final_parallaxed_position = mix(parallaxed_position, last_parallaxed_position, ratio);
    ...
    //     }
    // }
    float sun_closest_depth = texture2D(u_sun_shadow_map_texture, sun_shadow_map_position.xy).r;
    float shadow = floor(1.0 + sun_closest_depth + bias - sun_current_depth);
    ...
    float grass_patch_ = smooth_round(grass_patch, 0.01);
    float light_ = smooth_round(clamp(light * shadow, 0.0, 1.0), 0.25);
    vec3 color = (
        (
    ...
    // color = mix(color, vec3(position.y / MAX_HEIGHT), 0.5);
