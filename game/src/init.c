...
#include "grass_shader/grass_shader.h"
...
mesh_t* plain_mesh = NULL;

shader_t* grass_shader = NULL;
...
static err_t initialize_plain_mesh() {
    err_t err = NO_ERROR;
    ...
    RETHROW_IF_ERROR(generate_mesh(&plain_mesh, vbo_datas_arr, 3, indices_array, 6, 0, 0));

cleanup:
    return err;
}

static err_t initialize_man_mesh_and_animations() {
    err_t err = NO_ERROR;
...
err_t init() {
    ...
    RETHROW_IF_ERROR(initialize_plain_mesh());

    RETHROW_IF_ERROR(initialize_man_mesh_and_animations());
